...
async function main() {
   const packedBid = utils.solidityPack(["address", "address", "uint256", "uint256"], [bid.collectionAddress, bid.erc20Address, bid.tokenId, utils.parseEther(bid.bid.toString())])

   const bidHash = utils.keccak256(packedBid)

   const provider = new providers.JsonRpcProvider('https://rpc.sepolia.org/', 11155111);

   const bidderSigner = new Wallet(process.env.BIDDER_PRIVATE_KEY as string, provider)

   const bidderSignatureOfBid = await bidderSigner.signMessage(bidHash)

   const ownerSigner = new Wallet(process.env.MARKETPLACE_PRIVATE_KEY as string, provider)

   const hashedBidderSig = utils.keccak256(bidderSignatureOfBid)

   const ownerSignatureApproval = await ownerSigner.signMessage(utils.arrayify(hashedBidderSig))
